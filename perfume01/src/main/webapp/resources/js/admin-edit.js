$(function(){
	...
	//닉네임 검사
	$("[name=member_nick]").blur(function(){
		...
		$.ajax({
			...
			success:function(response){
				if(response == "Y") {
					...
					target.removeClass("valid invalid invalid2")
								.addClass("valid");
					target.val(`${requestScope.mlist.member_nick}`);
				}
				else {
	...
	//주소 검사
	$("[name=member_detail_addr]").blur(function(){
		var memberPost = $("[name=member_post]").val();
		var memberBasicAddr = $("[name=member_basic_addr]").val();
