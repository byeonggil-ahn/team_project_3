...
$(function() {
	// 카테고리별 개수
	$.ajax({
		url:contextPath + "/rest/productManage/categoryCount",
		method:"get",
		success:function(response) {
			...
			new Chart(ctx, {
				...
				data: {
					...
					labels: ['아쿠아 마린', '아로마틱', '앰버', '우디', '달콤', '스모키', '과일', '그린'],
					datasets: [{
						label: '상품 개수',
						data: response.cnts,
						borderWidth: 1,
	...
	// 카테고리별 판매량
	$.ajax({
		url:contextPath + "/rest/productManage/categorySellCount",
		method:"get",
		success:function(response) {
	...
	// 카테고리별 가격
	$.ajax({
		url:contextPath + "/rest/productManage/categoryPrice",
		method:"get",
		success:function(response) {
